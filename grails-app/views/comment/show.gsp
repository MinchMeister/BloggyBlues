			...
			<ol class="property-list comment">
				...
				<li class="fieldcontain">
					<span id="entry-label" class="property-label"><g:message code="comment.entry.label" default="Entry" /></span>
						...
						<span class="property-value" aria-labelledby="entry-label"><g:link controller="bloggyBlues" action="show" id="${commentInstance?.entry?.id}">${commentInstance?.entry?.encodeAsHTML()}</g:link></span>
